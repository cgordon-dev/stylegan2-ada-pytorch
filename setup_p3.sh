...
# Production script for setting up a p3.2xlarge (NVIDIA V100) instance
# for running StyleGAN2-ADA-PyTorch.
#
# This script installs:
#   - NVIDIA drivers (using ubuntu-drivers autoinstall)
#   - CUDA Toolkit 10.1 (runfile installation, driver installation skipped)
#   - cuDNN 7.6.5 (expects file at /tmp/cudnn-10.1_7.6.5-1_amd64.deb)
#   - Required build tools and Python dependencies
#   - A Python virtual environment with PyTorch 1.7.1 (cu101)
#
# Assumptions:
#   - The system is Ubuntu (20.04/22.04)
#   - This script is run as root (via sudo)
#   - The cuDNN .deb file for CUDA 10.1 is placed in /tmp
...
CUDA_RUNFILE_URL="https://developer.download.nvidia.com/compute/cuda/10.1/Prod/local_installers/cuda_10.1.105_418.39_linux.run"
CUDA_RUNFILE="/tmp/cuda_10.1.105_418.39_linux.run"

if [ ! -f "$CUDA_RUNFILE" ]; then
    log "Downloading CUDA Toolkit 10.1 runfile..."
    wget -O "$CUDA_RUNFILE" "$CUDA_RUNFILE_URL"
fi

chmod +x "$CUDA_RUNFILE"

log "Installing CUDA Toolkit 10.1 (skipping driver installation)..."
"$CUDA_RUNFILE" --silent --toolkit --override

# Install cuDNN 7.6.5 for CUDA 10.1
CUDNN_DEB="/tmp/cudnn-10.1_7.6.5-1_amd64.deb"
if [ -f "$CUDNN_DEB" ]; then
    log "Installing cuDNN 7.6.5..."
    dpkg -i "$CUDNN_DEB"
else
...
log "Installing PyTorch (with CUDA 10.1 support)..."
pip install torch==1.7.1+cu101 torchvision torchaudio -f https://download.pytorch.org/whl/torch_stable.html
...
log "Setup for p3.2xlarge (NVIDIA V100) completed successfully."